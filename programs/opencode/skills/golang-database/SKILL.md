...
Go's `database/sql` provides a solid foundation for database access. Use `sqlx`, `pgx`, or `sqlc` on top of it for ergonomics — never an ORM. `sqlc` generates type-safe Go code from SQL queries at build time, pairing naturally with `pgx` as the underlying driver.

When using sqlx, pgx, or sqlc, refer to the library's official documentation and code examples for current API signatures.

## Best Practices Summary

1. **Use sqlx, pgx, or sqlc — not ORMs** — ORMs hide SQL, generate unpredictable queries, and make debugging harder. `sqlc` is preferred when you want compile-time type safety from hand-written SQL
...
| `sqlc` | Type-safe SQL, codegen | Generated structs + pgx | Yes (uses pgx by default) |
...
**Why sqlc fits this philosophy:**

- You write real SQL — no DSL, no query builder, no magic
- `sqlc generate` produces type-safe Go code with proper structs and method signatures
- The generated code uses `pgx` directly — you get full PostgreSQL feature support
- Queries are visible, reviewable, and version-controlled as `.sql` files
- Compile-time errors catch mismatches between queries and Go types
...
### sqlc — Parameterized by Default

With sqlc, you write queries in `.sql` files and the generated Go code is always parameterized:
...
```go
// Generated by sqlc — parameters are type-safe, injection is impossible
user, err := queries.GetUserByEmail(ctx, email)
```

sqlc also handles `IN` clauses and dynamic queries safely through its query annotations. See [sqlc documentation](https://docs.sqlc.dev/) for supported query patterns.

## Struct Scanning and NULLable Columns

Use `db:"column_name"` tags for sqlx, `pgx.CollectRows` with `pgx.RowToStructByName` for pgx. With sqlc, structs are generated automatically from your schema and queries — you do not write or tag them manually. Handle NULLable columns with pointer fields (`*string`, `*time.Time`) — they work cleanly with both scanning and JSON marshaling. sqlc handles NULLable columns by generating pointer fields or `sql.NullXxx` types depending on configuration. See [Scanning Reference](./references/scanning.md) for examples of all approaches.
...
- [sqlc](https://sqlc.dev/) — [docs](https://docs.sqlc.dev/) — [GitHub](https://github.com/sqlc-dev/sqlc)
