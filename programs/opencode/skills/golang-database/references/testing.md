...
sqlc generates the query layer — you still test it through repository interfaces. The generated `Queries` struct accepts a `pgx.Tx` or `*pgxpool.Pool`, so integration tests use the same transaction-rollback pattern:
...
func (s *UserRepoSuite) SetupTest() {
    ...
    s.Require().NoError(err)
    s.tx = tx
    s.queries = db.New(tx) // db is the generated sqlc package
}
...
For unit tests, wrap sqlc's `Queries` behind a repository interface so business logic can be tested with mocks — do not mock the generated code directly.
